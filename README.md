...
flowchart TD
    ...
    subgraph GEN["两阶段草稿生成 Two-Draft Generation"]
        direction LR
        TC["`**tmp_claude**
        直觉草稿 Intuitive Draft
        ────────────────
        Role I 生成
        ✗ 无 skill-creator 规范
        *Generated by Role I*
        *No skill-creator spec*`"]

        TS["`**tmp_standard**
        规范实现 Spec-Compliant Draft
        ────────────────
        Role II 生成
        ✓ 按 skill-creator 规范从零实现
        *Generated by Role II*
        *Follows skill-creator spec*`"]
    end

    BLIND["`**盲评对比 Blind Comparison**
    Role III — 不知道版本身份
    *Role III — identity hidden*`"]

    EVAL["`**动态 Eval 设计 Dynamic Eval Design**
    Role I 全新实例 — 仅知需求规格
    *Fresh Role I instance — requirement only*`"]

    subgraph ITER["迭代测试循环 Iteration Loop"]
        direction TB
        TEST["`**自动测试 Auto-Test**
        Role III 逐条独立打分
        *Role III scores each assertion independently*`"]
        HUMAN["`**人工确认节点 Human Checkpoint**
        通过率 + 最薄弱3点 + 改进摘要
        *Pass rate · 3 weakest · improvement summary*`"]
        IMPROVE["`**角色II改进 Role II Improves**
        全新实例 + 提炼后改进指令
        *Fresh instance + distilled instructions*`"]
        TEST --> HUMAN
        HUMAN -->|继续 Continue| IMPROVE
        IMPROVE --> TEST
    end

    CONVERGE{"`**收敛？Converged?**
    ≥85% AND Δ<5%×2`"}

    FINAL["`**✅ 成品 Skill 交付**
    Final Skill Delivered`"]
    ...
    CONVERGE -->|Yes| FINAL
    CONVERGE -->|No — 局部最优/硬上限
Local optimum / hard cap| FINAL

    style TC fill:#dbeafe,stroke:#3b82f6,color:#1e3a5f
    style TS fill:#dcfce7,stroke:#22c55e,color:#14532d
    style BLIND fill:#fef9c3,stroke:#eab308,color:#713f12
    style EVAL fill:#fef9c3,stroke:#eab308,color:#713f12
    style TEST fill:#f3e8ff,stroke:#a855f7,color:#3b0764
    style HUMAN fill:#ffe4e6,stroke:#f43f5e,color:#881337
    style IMPROVE fill:#dcfce7,stroke:#22c55e,color:#14532d
    style CONVERGE fill:#fed7aa,stroke:#f97316,color:#431407
    style FINAL fill:#d1fae5,stroke:#10b981,color:#064e3b
