...
|---------------|--------------------------------|-----------------------------------|
| 认知隔离 Cognitive isolation | ✅ 真正空白上下文窗口 Genuine blank context window | ⚠️ 强制遗忘声明（近似）Forced-forgetting declaration (approximate) |
| 两阶段对比价值 Draft comparison | ✅ 完整 Full value | ⚠️ 有限（同实例生成）Limited (same instance) |
| 并行执行 Parallel execution | ✅ 3-A 和 3-B 并行 3-A and 3-B run in parallel | ❌ 串行 Serial |
| 文件持久化 File persistence | ✅ 文件系统 Filesystem | ✅ Artifacts storage |
| 交付方式 Delivery | ✅ bash 文件操作 bash file ops | ✅ present_files 下载 download |
| 收敛判断 Convergence logic | ✅ 完整 Full | ✅ 完整 Full |
| 人工确认节点 Human checkpoints | ✅ 完整 Full | ✅ 完整 Full |
...
- [Claude Code](https://claude.ai/code) 已安装 / installed
- [skill-creator](https://github.com/anthropics/skill-creator) 已安装（skill-cooker 依赖它）/ installed (skill-cooker depends on it)
- Claude Pro 或 Max 订阅，或 Anthropic API Key / Claude Pro or Max subscription, or Anthropic API Key
...
9 explicit constraints drive skill-cooker's behavior, ensuring predictable and repeatable results:

1. **环境前置检查** — 环境不满足时立即中止，不静默降级 / Environment pre-check — abort immediately if requirements unmet, no silent degradation
2. **最低可执行性预检** — tmp_claude 必须有 name、description 和行为步骤 / Minimum viability pre-check — tmp_claude must have name, description, and at least one behavioral step
3. **Assertion 数量门槛** — 少于 5 条时强制补充 / Assertion count threshold — force supplementation if fewer than 5
4. **双收敛条件** — 通过率 ≥ 85% AND 连续两轮改善 < 5% / Dual convergence condition — pass rate ≥ 85% AND two consecutive rounds improvement < 5%
5. **局部最优处理** — 停滞时报告状态，不强制继续 / Local optimum handling — report stall and ask user to decide, no forced continuation
6. **最大迭代硬上限** — 5 轮后暂停，请用户决策 / Hard iteration cap — pause after 5 rounds, user decides next step
7. **结构化 diff 呈现** — 被删除内容单独列出请用户确认 / Structured diff presentation — removed content listed separately for user confirmation
8. **文件操作原子性** — 备份验证后再删除，冲突检查前置 / Atomic file operations — backup and verify before delete, conflict check upfront
9. **subagent 上下文边界显式声明** — 每个角色的允许/禁止内容明确定义 / Explicit subagent context boundaries — allowed and forbidden content defined per role
...
```
skill-cooker/
├── SKILL.md                    # 主 skill 文件（中文版）/ Main skill file (Chinese)
├── SKILL.en.md                 # 英文版 / English version
└── references/
    ├── roles.md                # 三角色上下文边界清单 / Context boundary rules for all three roles
    ├── eval-schema.md          # eval 格式规范 / Eval format specification
    └── convergence.md          # 收敛判断完整逻辑 / Full convergence calculation logic
```
